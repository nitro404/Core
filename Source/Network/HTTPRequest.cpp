...
bool HTTPRequest::startTransfer(const HTTPConfiguration & configuration, HTTPUtilities::CURLMultiHandle & curlMultiHandle) {
	...
	if(requestType.value() == CURLOPT_CUSTOMREQUEST) {
		if(!HTTPUtilities::isSuccess(curl_easy_setopt(m_curlEasyHandle.get(), CURLOPT_CUSTOMREQUEST, Utilities::toUpperCase(magic_enum::enum_name(m_method)).c_str()), fmt::format("Failed to set cURL request #{} to custom type '{}'.", m_id, magic_enum::enum_name(m_method)))) {
			return false;
		}
	}
	else {
