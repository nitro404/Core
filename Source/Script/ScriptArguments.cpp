...
const char ScriptArguments::COND_OPEN_CHAR = '<';
const char ScriptArguments::COND_CLOSE_CHAR = '>';
const char ScriptArguments::COND_OPTION_CHAR = '|';
const char ScriptArguments::ARG_CHAR = ':';
...
std::string ScriptArguments::applyConditionals(const std::string & command) const {
	...
	for(size_t i = 0; i < command.length(); i++) {
		if(command[i] == COND_OPEN_CHAR) {
			depth++;
			brackets++;
		}
		else if(command[i] == COND_CLOSE_CHAR) {
			depth--;
			brackets++;
		}
	}
...
std::string ScriptArguments::applyConditionalsHelper(const std::string & command) const {
	...
	for(size_t i = 0; i < command.length(); i++) {
		...
		else if(command[i] == COND_OPTION_CHAR) {
			if(start >= 0 && opt < 0) {
				opt = i;
			}
		}
	...
	for(size_t i = 0; i < newCommand.length(); i++) {
		if(newCommand[i] == COND_OPEN_CHAR) {
			brackets++;
		}
		else if(newCommand[i] == COND_CLOSE_CHAR) {
			brackets++;
		}
	}
	...
	for(size_t i = 0; i < argumentParts.size(); i++) {
		...
		if(j < arguments.size()) {
			if(hasArgument(arguments[j])) {
				...
				newCommand << fmt::format("{0}{1}{0}", ARG_CHAR, arguments[j]);
			}
